...
<p>Instead, we want to clone TerriaJS from its <a href="https://github.com/TerriaJS/terriajs">GitHub repo</a> and use that in our TerriaMap build.  Traditionally, <code>npm link</code> is the way to do this.  However, we do not recommend use of <code>npm link</code> because it frequently leads to multiple copies of some libraries being installed, which in turn leads to all sorts of frustrating build problems.  Instead, we recommend <a href="https://www.npmjs.com/package/npmgitdev">npmgitdev</a>.  <code>npmgitdev</code> lets us safely clone a git repo into our <code>node_modules</code> directory and use it pretty much as if npm had put it there itself.</p>
<p>First, install <code>npmgitdev</code> globally:</p>
<pre><code>npm install -g npmgitdev
</code></pre>

<p>Then, in your TerriaMap directory, remove the existing <code>terriajs</code> package, and clone the terriajs repo there instead.</p>
...
<p>Now, if you run <code>npm install</code>, <code>npm</code> will fail with an error like this:</p>
<pre><code>npm ERR! git C:\github\TerriaMap\node_modules\terriajs: Appears to be a git repo or submodule.
npm ERR! git     C:\github\TerriaMap\node_modules\terriajs
npm ERR! git Refusing to remove it. Update manually,
npm ERR! git or move it out of the way first.
</code></pre>

<p>This is <em>good</em>!  It means <code>npm</code> recognizes that your <code>terriajs</code> directory is now a git repo, and it is refusing to touch it out of fear of making you lose your work.  This is where <code>npmgitdev</code> comes in:</p>
<pre><code>npmgitdev install
</code></pre>

<p><code>npmgitdev</code> is a wrapper around <code>npm</code> that, for each git repo in <code>node_modules</code>:</p>
<ul>
<li>Makes sure your working directory is clean (i.e. you have no uncommitted changes).</li>
<li>Moves the <code>.git</code> directory out of the way so that <code>npm</code> can't see it or clobber it.</li>
<li>Copies all the <code>devDependencies</code> in <code>package.json</code> into <code>dependencies</code>.  This way npm will install all your dev-time stuff too.</li>
<li>Runs <code>npm</code> normally.</li>
<li>Moves the <code>.git</code> directory back.</li>
</ul>
<div class="admonition note">
<p class="admonition-title">Note</p>
<p>If you hit CTRL-C while <code>npmgitdev</code> is running, it will be unable to clean up after itself automatically.  Look for a <code>npmgitdev-XXXXX</code> directory.  The <code>mappings.json</code> file in that directory contains a record of the changes that <code>npmgitdev</code> made so that you can undo them manually.  Usually this just means moving the <code>.git</code> directory back.</p>
</div>
<p>Now, we can edit TerriaJS in <code>node_modules/terriajs</code> with the benefit of a full-feature git repo.</p>
...
<p>The <code>package.json</code> in the <code>master</code> branch of TerriaMap should point to official releases of <code>terriajs</code> on npm, NOT GitHub branches.</p>
...
<p>You need a standalone install of MkDocs in order to build the user guide, see <a href="http://www.mkdocs.org/#installation">http://www.mkdocs.org/#installation</a> for details.</p>
