...
<html class="no-js">
  <head>
      ...
      <meta name="generator" content="mkdocs+mkdocs-material#1.0.3">
        ...
        <title>Deploying TerriaMap - TerriaJS</title>
...
<div class="md-search" data-md-component="search">
  ...
  <li class="md-nav__item">
    ...
    <a href="./" title="Deploying TerriaMap" class="md-nav__link md-nav__link--active">
      Deploying TerriaMap
    </a>
...
<nav class="md-nav md-nav--secondary">
    ...
    <ul class="md-nav__list" data-md-scrollfix>
        ...
        <li class="md-nav__item">
  <a href="#using-the-included-nodejs-based-web-server" title="Using the included Node.js-based web server" class="md-nav__link">
    Using the included Node.js-based web server
        ...
        <li class="md-nav__item">
  <a href="#using-any-web-server" title="Using any web server" class="md-nav__link">
  ...
  <li class="md-nav__item">
      ...
      <a href="../deploying-with-docker/" title="Deploying with Docker" class="md-nav__link">
  ...
  <li class="md-nav__item">
      ...
      <a href="../setting-up-geoserver/" title="Setting Up Geoserver" class="md-nav__link">
    ...
    Using any web server
          ...
          <div class="md-content">
            <article class="md-content__inner md-typeset">
                  ...
                  <h1>Deploying TerriaMap</h1>
                
                <p>TerriaMap can be deployed in almost any environment.</p>
<p>First, you may want to build a minified version of TerriaMap by running:</p>
<pre><code>npm run gulp release
</code></pre>

<p>The normal build (<code>npm run gulp</code>) can be deployed as well, but the release version is smaller and faster.</p>
<p>Then, you can host your TerriaMap using either the included Node.js-based web server, or by using any web server of your choosing.</p>
<h3 id="using-the-included-nodejs-based-web-server">Using the included Node.js-based web server</h3>
<p>The easiest way to deploy your TerriaMap is to use the included Node.js-based web server, called <a href="https://github.com/TerriaJS/terriajs-server">terriajs-server</a>.  You'll need Node.js 4.0+ installed on the server in order to run terriajs-server.  </p>
<div class="admonition note">
<p class="admonition-title">Note</p>
<p>Node.js 5.0+ is required to <em>build</em> TerriaMap, but only 4.0+ is required to <em>run</em> it.</p>
</div>
<p>Then, copy the following files and directories from your local system where you built TerriaMap onto the server:</p>
<ul>
<li><code>wwwroot</code></li>
<li><code>node_modules</code></li>
<li><code>devserverconfig.json</code></li>
</ul>
<p>And on the server, change to the directory where you copied those files and directories and run:</p>
<pre><code>./node_modules/terriajs-server/run_server.sh --config-file devserverconfig.json
</code></pre>

<p>The server will start on port 3001.  You can specify a different port by adding <code>--port 1234</code> to the command-line above.</p>
<p>It is usually a good idea to run another web server, such as <a href="https://nginx.org/en/">nginx</a> or <a href="https://varnish-cache.org/">Varnish</a> on port 80 and then reverse-proxy to the Node.js server, rather than running terriajs-server on port 80 directly.   You will find configuration files for Varnish with the TerriaMap source code in the <a href="https://github.com/TerriaJS/TerriaMap/tree/master/varnish">varnish directory</a>.  In addition to acting as a reverse proxy for the Node.js server, the supplied Varnish configuration also caches requests to proxied map data in order to improve performance.</p>
<h3 id="using-any-web-server">Using any web server</h3>
<p><a href="https://github.com/TerriaJS/terriajs-server">terriajs-server</a>, described above, only does a few things:</p>
<ol>
<li>It serves up the static HTML, JavaScript, and CSS that make up the application.</li>
<li>It includes a simple service at <code>/proxy</code> that allows TerriaJS to access geospatial data servers that don't support <a href="../../connecting-to-data/cross-origin-resource-sharing/">CORS</a>.  If this service is not available, TerriaJS won't be able to access any datasets that are on other servers and that don't support CORS.</li>
<li>It includes another service at <code>/convert</code> that uses <a href="http://www.gdal.org/">GDAL</a> and OGR to transform otherwise unsupported geospatial vector data (e.g. shapefiles) to GeoJSON for display by the TerriaJS client.  If this service is not available, these data formats will not be supported.  However, all the <a href="../../connecting-to-data/catalog-items/">formats that TerriaJS supports directly</a> will work just fine.</li>
<li>When configured correctly, it persists blobs of JSON for use in the sharing feature.  If this service is not available, the JSON can be stored in the share URL, instead.  However, this makes for some extremely long URLs.</li>
</ol>
<p>If these limitations are acceptable, you can run your TerriaMap on virtually any web server by simply copying the TerriaMap <code>wwwroot</code> onto the server!</p>
<p>You can also incrementally add these services to your own server, as necessary, by porting the code in <a href="https://github.com/TerriaJS/terriajs-server">terriajs-server</a> to your environment.</p>
...
<footer class="md-footer">
    ...
    <div class="md-footer-nav">
      <nav class="md-footer-nav__inner md-grid">
          ...
          <a href="../deploying-to-aws/" title="Deploying to AWS" class="md-flex md-footer-nav__link md-footer-nav__link--next" rel="next">
            <div class="md-flex__cell md-flex__cell--stretch md-footer-nav__title">
              <span class="md-flex__ellipsis">
                <span class="md-footer-nav__direction">
                  Next
