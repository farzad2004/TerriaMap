...
<html class="no-js">
  <head>
      ...
      <meta name="generator" content="mkdocs+mkdocs-material#1.0.3">
        ...
        <title>Deploying with Docker - TerriaJS</title>
...
<div class="md-search" data-md-component="search">
  ...
  <li class="md-nav__item">
    ...
    <a href="./" title="Deploying with Docker" class="md-nav__link md-nav__link--active">
      Deploying with Docker
    </a>
...
<p>We haven't used Docker for TerriaMap deployments in quite some time, so the instructions below are likely to be incorrect or outdated.  If you try it, we'd like to hear about your experiences on the <a href="https://groups.google.com/forum/#!forum/terriajs">forum</a></p>
</div>
<p>If you intend to deploy using docker, Dockerfiles have been provided for nationalmap and varnish. There are currently no hosted builds of these images. To build and deploy these docker images locally:</p>
...
<p>This exposes varnish on port 80. Port 3001 for nationalmap remains behind docker's NAT. If you wish to interact with it directly change</p>
<pre><code>-p &lt;internal port&gt;
</code></pre>

<p>with</p>
<pre><code>-p &lt;external port&gt;:&lt;internal port&gt;
</code></pre>

<p>for this component.</p>
<p>There is also a docker_release.sh bash script available that will carry out these commands and post to an AWS instance.  If you are not deploying on AWS you can use the script as a starting point for creating your own script.</p>
<pre><code>    Syntax: deploy_release pem_file host_name_or_ip
    example: deploy_release login.pem 54.79.62.244
</code></pre>

<p>Some handy commands:</p>
<pre><code class="bash"># If you want to avoid using sudo on every docker command (log out and log in after)
...
<p>If you're deploying varnish to an environment with less than 25GB on the root file system, or for whatever reason you want to specify the location of the cache file e.g. using an EBS volume, you can set the path for that:</p>
<pre><code>docker run -d -p 80:80 --name varnish -v &lt;cache-dir&gt;:/mnt/cache/varnish --link nationalmap:nm nationalmap/varnish
</code></pre>

<p>The current release with ubuntu 14.04 is rather old and is missing some useful commands.  To update to the most recent of release of Docker, use the commands below from <a href="http://www.ubuntuupdates.org/ppa/docker?dist=docker">here</a>.</p>
<pre><code>sudo apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 36A1D7869245C8950F966E92D8576A8BA88D21E9
...
<footer class="md-footer">
    ...
    <div class="md-footer-nav">
      <nav class="md-footer-nav__inner md-grid">
          ...
          <a href="../deploying-to-aws/" title="Deploying to AWS" class="md-flex md-footer-nav__link md-footer-nav__link--prev" rel="prev">
            <div class="md-flex__cell md-flex__cell--shrink">
          ...
          <a href="../setting-up-a-region-mapping-server/" title="Setting Up a Region Mapping Server" class="md-flex md-footer-nav__link md-footer-nav__link--next" rel="next">
            <div class="md-flex__cell md-flex__cell--stretch md-footer-nav__title">
              <span class="md-flex__ellipsis">
                <span class="md-footer-nav__direction">
                  Next
