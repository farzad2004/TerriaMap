...
<html class="no-js">
  <head>
      ...
      <meta name="generator" content="mkdocs+mkdocs-material#1.0.3">
        ...
        <title>Initialization Files - TerriaJS</title>
...
<div class="md-search" data-md-component="search">
  ...
  <li class="md-nav__item">
    ...
    <a href="./" title="Initialization Files" class="md-nav__link md-nav__link--active">
      Initialization Files
    </a>
...
<nav class="md-nav md-nav--secondary">
    ...
    <ul class="md-nav__list" data-md-scrollfix>
        ...
        <li class="md-nav__item">
  <a href="#using-a-catalog-file" title="Using a catalog file" class="md-nav__link">
    Using a catalog file
        ...
        <li class="md-nav__item">
  <a href="#editing-catalog-files" title="Editing catalog files" class="md-nav__link">
    Editing catalog files
        ...
        <li class="md-nav__item">
  <a href="#catalog-file-properties" title="Catalog file properties" class="md-nav__link">
    ...
    <nav class="md-nav">
      <ul class="md-nav__list">
          ...
          <li class="md-nav__item">
  <a href="#corsdomains" title="corsDomains" class="md-nav__link">
    corsDomains
  </a>
          ...
          <li class="md-nav__item">
  <a href="#homecamera-and-initialcamera" title="homeCamera and initialCamera" class="md-nav__link">
    ...
    <nav class="md-nav">
      <ul class="md-nav__list">
          ...
          <li class="md-nav__item">
  <a href="#option-1-north-south-east-west" title="Option 1: north, south, east, west" class="md-nav__link">
    Option 1: north, south, east, west
          ...
          <li class="md-nav__item">
  <a href="#option-2-position-direction-and-up" title="Option 2: position, direction and up" class="md-nav__link">
    Option 2: position, direction and up
          ...
          <li class="md-nav__item">
  <a href="#option-3-positionheading-like-an-aircraft" title="Option 3: positionHeading (like an aircraft)" class="md-nav__link">
    Option 3: positionHeading (like an aircraft)
          ...
          <li class="md-nav__item">
  <a href="#option-4-lookat-a-feature" title="Option 4: lookAt (a feature)" class="md-nav__link">
    ...
    Option 4: lookAt (a feature)
          ...
          <div class="md-content">
            <article class="md-content__inner md-typeset">
                ...
                <p>A catalog in TerriaJS is defined in one or more "initialization files" (or init files).  In a default TerriaMap installation, the main init file is found in <code>wwwroot/init/terria.json</code>.</p>
<p>An init file is a <a href="https://en.wikipedia.org/wiki/JSON">JSON file</a> with this basic structure:</p>
<pre><code>{
    &quot;catalog&quot;: [
        {
            &quot;type&quot;: &quot;group&quot;,
...
<li><a href="../../connecting-to-data/catalog-groups/">Catalog Group</a>: A group (folder) of items.  Different group types allow the contents to be manually specified or to be automatically determined by querying various types of server.</li>
<li><a href="../../connecting-to-data/catalog-items/">Catalog Item</a>: Actual geospatial or chart data from a file or service, in various formats.</li>
<li><a href="../../connecting-to-data/catalog-functions/">Catalog Function</a>: A parameterized service, such as a Web Processing Service (WPS).  The user supplies the parameters and gets back some result.</li>
</ul>
</li>
</ul>
<p>Most of the other properties of each layer depend on the specific type.  See the links above for details of each type.</p>
<h3 id="using-a-catalog-file">Using a catalog file</h3>
<p>There are four ways to load a catalog file into a TerriaJS application:</p>
<ol>
<li>Store it in Terria Map's <code>wwwroot/init</code> directory, and refer to it in the <code>initializationUrls</code> section of the <a href="../client-side-config/"><code>config.json</code></a> file. It is loaded automatically when you visit the webpage.  This is how <code>wwwroot/init/terria.json</code> is loaded in the default TerriaMap setup.</li>
<li>Store it in Terria Maps's <code>wwwroot/init</code> directory, without adding it to config.json. Add the catalog file name (without <code>.json</code>) to the URL after <code>#</code>. For instance, <code>example.com/terria#mycatalog</code>. See <a href="../../deploying/controlling-with-url-parameters/">Controlling with URL Parameters</a> for more information. This method is useful when developing a catalog that is not quite ready for public access, but it is helpful to show it to interested stakeholders.</li>
<li>Store it anywhere on the web (on a <a href="../../connecting-to-data/cross-origin-resource-sharing/">CORS-enabled</a> server). Add the complete URL (including <code>.json</code>) to the URL, after <code>#</code>. For instance, <code>http://nationalmap.gov.au/#http://example.com/mycatalog.json</code>. This method is useful when developing services for a TerriaJS instance which you don't directly control, and for rapidly previewing changes which you can also share with people.</li>
<li>Store it locally, then drag and drop it into the Terria Map window.</li>
</ol>
<p>All catalog files, however loaded, are merged together in TerriaJS. Any two items with the same name and place in the tree are combined. This means that if two catalog files each define a group called "Water", there will be only one "Water" group in Terria, containing the two sets of group members merged together.</p>
<h3 id="editing-catalog-files">Editing catalog files</h3>
<p>Catalog files can be edited three ways:</p>
<ol>
<li>Using a desktop text editor. Be very careful to ensure that your file is valid JSON. This is more restrictive format than simple JavaScript, for instance. You can use <a href="http://jsonlint.com/">http://jsonlint.com/</a>.</li>
<li>Using a JSON-specific editor, such as <a href="http://www.jsoneditoronline.org/">http://www.jsoneditoronline.org/</a>. This has the advantage that your file will be valid JSON.</li>
<li>Using the TerriaJS Catalog Editor, currently available in a preview version at <a href="http://terria.io/DataSourceEditor/">http://terria.io/DataSourceEditor/</a>. This editor is not yet considered reliable, and may cause data corruption.</li>
</ol>
<h2 id="catalog-file-properties">Catalog file properties</h2>
<h3 id="corsdomains"><code>corsDomains</code></h3>
<p>By default, TerriaJS proxies all requests within the proxy whitelist specified in the <a href="../server-side-config/">Server-side Config</a>, making the assumption that the servers do not support CORS. You can add hosts that are known to support CORS to this property to avoid proxying them.</p>
<p><code>"corsDomains": [ "myserver.gov.au" ]</code></p>
<p>See <a href="../../connecting-to-data/cross-origin-resource-sharing/">Cross-Origin Resource Sharing</a> for more information.</p>
...
<p>The bounding box method uses <code>north</code>, <code>east</code>,<code>south</code>, and <code>west</code>, in lat/lng decimal degrees.  The camera will be positioned in the center point of those bounds, looking toward the Earth's center, zoomed back enough to see to the edges of the bounds.</p>
...
<p>For most purposes positioning this way is difficult for normal
...
<ul>
<li><code>targetLongitude</code>: The longitude to look at</li>
<li><code>targetLatitude</code>: The latitude to look at</li>
<li><code>targetHeight</code>: in meters above the WGS84 ellipsoid (positive is up)</li>
...
<li><code>range</code>: in meters from the thing you're looking at</li>
</ul>
<pre><code>&quot;homeCamera&quot;: {
    &quot;lookAt&quot;: {
        &quot;targetLongitude&quot;: 145,
...
<footer class="md-footer">
    ...
    <div class="md-footer-nav">
      <nav class="md-footer-nav__inner md-grid">
          ...
          <a href="../client-side-config/" title="Client-side Config" class="md-flex md-footer-nav__link md-footer-nav__link--prev" rel="prev">
            <div class="md-flex__cell md-flex__cell--shrink">
          ...
          <a href="../server-side-config/" title="Server-side Config" class="md-flex md-footer-nav__link md-footer-nav__link--next" rel="next">
            <div class="md-flex__cell md-flex__cell--stretch md-footer-nav__title">
              <span class="md-flex__ellipsis">
                <span class="md-footer-nav__direction">
                  Next
