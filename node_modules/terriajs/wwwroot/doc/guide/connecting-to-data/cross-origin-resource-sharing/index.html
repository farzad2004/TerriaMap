...
<html class="no-js">
  <head>
      ...
      <meta name="generator" content="mkdocs+mkdocs-material#1.0.3">
        ...
        <title>Cross-Origin Resource Sharing - TerriaJS</title>
...
<nav class="md-nav md-nav--secondary">
    ...
    <ul class="md-nav__list" data-md-scrollfix>
        ...
        <li class="md-nav__item">
  <a href="#more-detailed-explanation" title="More Detailed Explanation" class="md-nav__link">
    ...
    More Detailed Explanation
          ...
          <div class="md-content">
            <article class="md-content__inner md-typeset">
                ...
                <p>To ensure that TerriaJS is able to access your data, you must add all servers you intend to access to <code>devserverconfig.json</code>'s <code>allowProxyFor</code> list. This "whitelist" authorizes the proxy to work with those servers.</p>
<p>Failing to do this may result in an error like this:</p>
<p><code>...the server does not support CORS. If this is your server, verify that CORS is enabled and enable it if it is not. If you do not control the server, please contact the administrator of the server and ask them to enable CORS. Or ... ask us to add this server to the list of non-CORS-supporting servers that may be proxied.</code></p>
<p>Next, you should add servers that <em>do</em> support Cross-Origin Resource Sharing (CORS) to the <code>corsDomains</code> list in your <a href="../../customizing/initialization-files/">initialization file</a>. Servers in this list are contacted directly instead of going through the proxy... except:</p>
<ul>
<li>in Internet Explorer 9</li>
<li>if an HTTPS web page is accessing an HTTP server (this way we avoid a mixed content warning from the browser).</li>
</ul>
<p>If your server does <em>not</em> support CORS, then you still need to add it to the <code>allowProxyFor</code> whitelist, but do not add it to the <code>corsDomains</code> list. It will then be proxied.</p>
<p>Sometimes we deliberately exclude CORS-supporting servers from the <code>corsDomains</code> list; the proxy caches its data, so we leverage its caching to improve performance.</p>
<p>In both lists, a server name <code>foo.org</code> will be interpreted as <code>*.foo.org</code>. The port must match exactly. It's not very smart, so if you specify port 80 that won't match a server without a port specified, and vice-versa.</p>
<p>The downside to a permissive whitelist is that you'll proxy for more servers, and people could use your proxy to make their (malicious?) traffic look like it's coming from your server instead of from theirs.</p>
<h2 id="more-detailed-explanation">More Detailed Explanation</h2>
<p>As a general rule, web browsers do not allow web sites running on one host (e.g. <code>nationalmap.gov.au</code>) from accessing data running on another host (e.g. <code>data.gov.au</code>) unless the data server (<code>data.gov.au</code> in our example) explicitly enables a feature called Cross-Origin Resource Sharing (CORS) by including a special HTTP header in its response.  Unfortunately, many servers still do not support CORS.  If you control the server you're trying to access data on, enabling CORS is almost certainly a good idea, and you can read more about how to do it at <a href="http://enable-cors.org/">enable-cors.org</a>.</p>
<p>TerriaJS has a trick to allow it to access data from a server even if that server doesn't support CORS: it uses a proxy server built into <a href="https://github.com/TerriaJS/terriajs-server">terriajs-server</a>, which is included with TerriaMap.  The idea is simple: we avoid the need for cross-origin support from servers by running a proxy on the same host that runs our app.  That proxy can make requests to <em>other</em> servers on our application's behalf.  This works because cross-origin restrictions are a security feature of web browsers, not web servers.  The proxy makes it look to the browser like the data comes from the same origin.  There's nothing dodgy about this.  The risks that browsers are defending against by imposing cross-origin restrictions are not present in this scenario.</p>
<p>However, by running a proxy server, our TerriaMap server potentially opens itself up to risks.  We don't want people to be able to bounce arbitrary requests through our proxy, because that could be used to make (malicious?) traffic look like it is coming from your server instead of from the real source.  This is why <code>terriajs-server</code> uses a whitelist of servers that it is willing to proxy for.  The <code>allowProxyFor</code> property in <code>devserverconfig.json</code> specifies the whitelist.  If the server is asked to proxy for any server not in the whitelist, the request will be rejected.</p>
...
<footer class="md-footer">
    ...
    <div class="md-footer-nav">
      <nav class="md-footer-nav__inner md-grid">
          ...
          <a href="../catalog-groups/" title="Catalog Groups" class="md-flex md-footer-nav__link md-footer-nav__link--next" rel="next">
            <div class="md-flex__cell md-flex__cell--stretch md-footer-nav__title">
              <span class="md-flex__ellipsis">
                <span class="md-footer-nav__direction">
                  Next
